...
public:
	inline SigSS7Sccp(SS7SCCP* sccp)
		: OnDemand(sccp? sccp->toString() : ""), m_sccp(sccp) {
		m_sccp->ref();
	}
	virtual ~SigSS7Sccp();
	virtual bool initialize(NamedList& params)
...
void SigChannel::handleEvent(SignallingEvent* event)
{
	...
	case SignallingEvent::Ringing:
		evRinging(event);
		break;
	case SignallingEvent::Circuit:
		evCircuit(event);
		break;
	case SignallingEvent::Generic:
		evGeneric(event,"transport");
		break;
	case SignallingEvent::Suspend:
		evGeneric(event,"suspend");
		break;
	case SignallingEvent::Resume:
		evGeneric(event,"resume");
		break;
	case SignallingEvent::Charge:
		evGeneric(event,"charge");
		break;
	default:
