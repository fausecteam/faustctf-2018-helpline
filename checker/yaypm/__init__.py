...
def embeddedStart(*args):
    ...
    """
    raise RuntimeError("Embedded Dispatcher should be run only from pymodule!")
...
class Dispatcher:
    ...
    def _fireHandlers(self, m, hdlr_type):
        ...
        if logger_messages.isEnabledFor(logging.DEBUG):
            ...
            for (name, type), handlers in self.handlers.items():
                result = result + "\n%s \"%s\": %d handler(s)" % \
...
class UnicodeEnabledTransferWrapper:
    ...
    def __getattribute__(self, item):
        try:
            ...
            transport = object.__getattribute__(self, 'transport')
            attr = object.__getattribute__(transport, item)
        return attr
...
class TCPDispatcher(Dispatcher, LineReceiver):
    ...
    def connectionMade(self):
        self.transport = UnicodeEnabledTransferWrapper(self.transport)
    ...
    def connectionLost(self, reason):
        ...
        for (m, (_, d)) in list(self.waiting.items()):
            try:
...
class TCPDispatcherFactory(ClientFactory):
    def __init__(self, connected,
                 args=[], kwargs={},
                 reenter=True, selfwatch=True, connectionLostHandler=None, clientConnectionFailedHandler=None):
        ...
        self.clientConnectionFailedHandler = clientConnectionFailedHandler
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        if self.connectionLostHandler:
            try:
                self.connectionLostHandler(connector, reason)
            except Exception as ex:
                logger.error("error, calling connectionLost handler: %s", ex)

    def clientConnectionFailed(self, connector, reason):
        if self.clientConnectionFailedHandler:
            try:
                self.clientConnectionFailedHandler(connector, reason)
            except Exception as ex:
                logger.error("error, calling clientConnectionFailedHandler: %s", ex)
                raise
        else:
            try:
                reactor.stop()
...
class Formatter(logging.Formatter):
    ...
    def format(self, record):
        if (record.levelname in Formatter._level_colors):
